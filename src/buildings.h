...
enum { // room object types
	...
	TYPE_SHELL_CASE, TYPE_PAN_SHELF, TYPE_JAR, TYPE_FOOD_TUB,
	...
	TYPE_JUMPSUIT, TYPE_HANDGUN, TYPE_SHOP_CART, TYPE_CASHREG, TYPE_FOOD_FISH, TYPE_KITCH_APP, TYPE_MILK, TYPE_RADIATOR, TYPE_RAD_FAN,
...
enum {/*building models*/ OBJ_MODEL_TOILET=0, OBJ_MODEL_SINK, OBJ_MODEL_TUB, OBJ_MODEL_FRIDGE, OBJ_MODEL_STOVE, OBJ_MODEL_TV, OBJ_MODEL_MONITOR/*unused*/, OBJ_MODEL_COUCH,
	...
	OBJ_MODEL_HANDGUN, OBJ_MODEL_SHOP_CART, OBJ_MODEL_CASHREG, OBJ_MODEL_FISH, OBJ_MODEL_CK_APP, OBJ_MODEL_MILK, OBJ_MODEL_RADIATOR, OBJ_MODEL_RAD_FAN,
...
struct room_object_t : public oriented_cube_t { // size=68
	...
	bool is_small_closet() const {return (type == TYPE_CLOSET && (item_flags == RTYPE_KITCHEN || get_width() < 1.2*dz()));} // includes pantry
	...
	unsigned get_pipe_dim()      const {return (dir ? 2 : unsigned(dim));} // encoded as: X:dim=0,dir=0 Y:dim=1,dir=0, Z:dim=x,dir=1
	float get_pipe_radius()      const {return 0.5*get_sz_dim((get_pipe_dim()+1)%3);}
...
public:
	...
	void add_ortho_cylin_to_verts(cube_t const &c, colorRGBA const &color, int dim, bool draw_bot, bool draw_top, int two_sided=0, bool inv_tb=0,
	...
	void add_vcylin_to_verts(cube_t const &c, colorRGBA const &color, bool draw_bot, bool draw_top, int two_sided=0, bool inv_tb=0,
		float rs_bot=1.0, float rs_top=1.0, float side_tscale=1.0, float end_tscale=1.0, bool skip_sides=0, unsigned ndiv=N_CYL_SIDES, float side_tscale_add=0.0,
		bool swap_txy=0, float len_tc2=1.0, float len_tc1=0.0, int half_or_quarter=0, int swap_end_txy=0);
	...
	void add_cylin_to_verts(point const &bot, point const &top, float bot_radius, float top_radius, colorRGBA const &color, bool draw_bot, bool draw_top,
		int two_sided=0, bool inv_tb=0, float side_tscale=1.0, float end_tscale=1.0, bool skip_sides=0, unsigned ndiv=N_CYL_SIDES, float side_tscale_add=0.0,
		bool swap_txy=0, float len_tc2=1.0, float len_tc1=0.0, int half_or_quarter=0, int swap_end_txy=0);
	void add_disk_to_verts(point const &pos, float radius, vector3d const &dir, colorRGBA const &color,
		bool swap_txy=0, bool inv_ts=0, bool inv_tt=0, unsigned ndiv=N_CYL_SIDES, float r_inner=0.0);
	void add_vert_disk_to_verts(point const &pos, float radius, bool normal_z_neg, colorRGBA const &color,
		bool swap_txy=0, bool inv_ts=0, bool inv_tt=0, unsigned ndiv=N_CYL_SIDES, float r_inner=0.0)
	{
		add_disk_to_verts(pos, radius, (normal_z_neg ? -plus_z : plus_z), color, swap_txy, inv_ts, inv_tt, ndiv, r_inner);
	}
...
struct room_t : public cube_t, public room_assignment_t { // size=56
	...
	void restore(room_assignment_t const &r) {*(room_assignment_t *)this = r;}
...
struct building_room_geom_t {
	...
	vect_cube_t light_bcubes, shelf_rack_occluders[2], glass_floors, jails; // shelf_rack_occluders: {back, top}
	...
	void add_pantry_shelf (room_object_t const &c);
	...
	void add_coat_rack(room_object_t const &c, float tscale);
	...
	void add_hard_hat_to_material(room_object_t const &c, rgeom_mat_t &mat);
	void add_hard_hat  (room_object_t const &c);
	void add_tophat    (room_object_t const &c);
	void add_comp_mouse(room_object_t const &c);
	...
	void add_ceil_tile(room_object_t const &c);
	...
	void add_cabinet_with_open_door(room_object_t const &c, cube_t const &door, colorRGBA const &color,
		float wall_thickness, unsigned front_face_mask, float z1_adj=0.0, float back_adj=0.0, float shelf_height=0.5);
	...
	void add_shower_head(room_object_t const &shower, float width, float wall_pos, float extent_amt, bool head_dim);
	...
	void add_gym_weight(room_object_t const &c);
	void add_food_tray(room_object_t const &c);
	...
	void expand_locker (room_object_t const &c, building_type_t btype);
...
struct elevator_t : public oriented_cube_t { // dim/dir applies to the door
	...
	bool is_moving=0, interior_room=0, in_backrooms=0, no_buttons=0, has_equip_room=0;
...
struct escalator_t : public oriented_cube_t { // Note: not yet used
	...
	float get_side_width () const {return 0.09*get_width();}
	float get_side_height() const {return 0.70*get_width();} // ramp/steps to top edge
	float get_upper_hang () const {return 0.22*get_width();}
...
struct building_interior_t {
	...
	//vect_room_object_t prev_objs; vector<room_t> prev_rooms; // used for debugging
	...
	bool obj_on_restaurant_counter(room_object_t const &obj) const;
...
struct building_t : public building_geom_t {
	...
	bool has_ind_info () const {return (interior && interior->ind_info);}
	...
	tid_nm_pair_t get_interior_ext_wall_texture() const;
	...
	void get_parking_str_wall_verts  (building_draw_t &bdraw) const;
...
public:
	...
	unsigned get_sub_room_containing_pt(point const &pt, unsigned parent_room_id) const;
...
private:
	...
	void add_wine_rack       (rand_gen_t&rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned objs_start);
	bool add_library_objs    (rand_gen_t rgen, room_t const &room, float &zval, unsigned room_id, float tot_light_amt, unsigned objs_start, bool is_basement, bool add_tables=0);
	...
	void add_prison_room(cube_t const &room, unsigned part_id, bool inc_half_walls, bool is_nested, bool is_hall=0);
	...
	void add_restaurant_objs(rand_gen_t rgen, room_t const &room, float zval, unsigned room_id, float light_amt, unsigned light_nx, unsigned light_ny);
	...
	void add_corner_trashcans(rand_gen_t &rgen, room_t const &room, float zval, unsigned room_id, float tot_light_amt, unsigned objs_start, bool dim, bool both_ends);
	...
	void add_parking_struct_objs(rand_gen_t rgen, room_t const &room, float zval, unsigned room_id, unsigned floor_ix,
		unsigned num_floors, unsigned &nlights_x, unsigned &nlights_y, float &light_delta_z, light_ix_assign_t &light_ix_assign);
	...
	void add_basement_room_detail_objs(rand_gen_t &rgen);
	bool place_book_on_obj   (rand_gen_t &rgen, room_object_t const &place_on, unsigned room_id, float tot_light_amt,
		unsigned objs_start, bool use_dim_dir, unsigned extra_flags=0, bool skip_if_overlaps=0, float rand_shift_amt=0.1);
	...
	bool place_milk_on_obj   (rand_gen_t &rgen, cube_t const &place_on, unsigned room_id, float tot_light_amt, vect_cube_t const &avoid=vect_cube_t());
	...
	void add_floor_stains    (rand_gen_t &rgen, cube_t const &place_area, float zval, unsigned room_id,
		float tot_light_amt, unsigned objs_start, unsigned num, float rmax, bool is_food=0);
	...
	bool apply_paint(point const &pos, vector3d const &dir, colorRGBA const &color, unsigned emissive_color_id, room_object const obj_type, point *hit_pos=nullptr) const;
...
void city_shader_setup(shader_t &s, cube_t const &lights_bcube, bool use_dlights, int use_smap, int use_bmap, float min_alpha=0.0, bool force_tsl=0,
	float pcf_scale=1.0, int use_texgen=0, bool indir_lighting=0, bool is_outside=1, bool enable_int_reflect=0, bool enable_dirt=0, bool enable_foam=0);
